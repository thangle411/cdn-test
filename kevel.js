...
` + u[a].replace(' at new ', ' at ');
  ...
  var Mv = rn.unstable_scheduleCallback,
    ...
    Fh = rn.unstable_ImmediatePriority,
...
` +
  ...
  on.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED = RR;
    ...
    (function () {
      ...
      var GS = function O(T) {
          ...
          function _c(r) {
            var o = tt[r];
            return function (s, f) {
              if (((s = mn(s)), (f = f == null ? 0 : yt(le(f), 292)), f && A0(s))) {
...
` +
  ...
  const tN = Object.freeze(
    ...
    A1 = oI(tN);
  var nN = self.fetch || (self.fetch = A1.default || A1),
  ...
  class i6 {
    async initializeServices(t) {
      const n = new e6(),
        ...
        a = new JE(),
        c = new QE();
      if (!(await i.getMainElement())) return;
      n.setMetaData({ appID: t.appID, uuid: t.uuid });
      const p = await c.getDeviceID(a, t.uuid);
      if (!p) return;
      n.setDeviceID(p);
      const y = new XN(n, a, {
        bannerCount: t.options.banner.count,
        formatACount: t.options.formatA.count,
        formatBCount: t.options.formatB.count,
      });
      if (!i || !l || !y || !a || !u || !c || !n) {
        console.error('Some services failed to initialize');
        return;
      }
      return { document: i, kevel: l, offers: y, cache: a, shoppingTrip: u, device: c, store: n };
    }
    async init(t) {
      try {
        const n = await this.initializeServices(t);
        if (!n) return;
        this.renderSkeletons(), await n.offers.init();
        const i = await n.offers.getOffers(n);
        if (!i) return;
